...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default='./dataset/limix/talent_csv', help='Specify the local storage directory of the dataset')
    ...
    classifier = LimiXPredictor(device=torch.device("cuda" if torch.cuda.is_available() else "cpu"),
        ...
        try:
            ...
            while sample_index == 0 or sample_index < search_space_sample_num:
                ...
                try:
                    t1 = time.time()
                    t2 = t1
                    rst, prediction_, testy = inference_dataset(classifier, le, scaler, X_train.copy(), y_train.copy(),
                ...
                if not (int(os.environ.get('WORLD_SIZE', -1)) > 0 and get_rank() != 0):
                    ...
                    pd.DataFrame(output_df).to_csv(os.path.join(save_root, rst['dataset name'] + '_pred_LimiX.csv'),
                                                   index=False)
                    del prediction_
            ...
            if args.debug and search_space_sample_num > 0 and len(aucs['sample_auc']) > 0:
                ...
                print(
                    f"[{idx}] {folder} -> default_auc: {aucs['default_auc']:.6f}, sample_auc: max: {np.max(aucs_list):.6f}, mean: {np.mean(aucs_list):.6f},  min: {np.min(aucs_list):.6f}")
